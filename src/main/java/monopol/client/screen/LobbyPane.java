...
import monopol.server.events.MegaEdition2PlayersEvents;
...
public class LobbyPane extends JLayeredPane {
    ...
    private void updateButtons(ArrayList<Client> clients, RootPane root, int players) {
        ...
        start.setFont(new Font(start.getFont().getName(), start.getFont().getStyle(), client.player().isHost && enoughPlayers() ? 50 : 42));
        start.setText(enoughPlayers() ? client.player().isHost ? "Starten" : "Warten auf Host" : "Zu wenig Spieler");
        start.setVisible(true);
    }

    private boolean enoughPlayers() {
        try {
            return client.serverMethod().enoughPlayers();
